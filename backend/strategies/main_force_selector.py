...
class MainForceStockSelector:
    ...
    def get_main_force_stocks(self, start_date: str = None, days_ago: int = None,
        ...
        try:
            ...
            queries = [
                ...
                f"所属同花顺行业，总市值，净利润，营收，市盈率，市净率，股息率，资产负债率，"
    ...
    def filter_stocks(self, df: pd.DataFrame, 
        ...
        if market_cap_cols:
            ...
            # 转换为数值（单位可能是亿或元），使用临时变量避免修改原数据单位
            temp_cap = pd.to_numeric(filtered_df[col_name], errors='coerce')
            
            # 判断单位（如果值很大，可能是元）
            max_val = temp_cap.max()
            if max_val > 100000:  # 大于10万，认为是元
                print(f"  检测到单位为元，比较时临时转换为亿")
                temp_cap = temp_cap / 100000000
            
            before = len(filtered_df)
            filtered_df = filtered_df[
                (temp_cap.notna()) & 
                (temp_cap >= min_market_cap) &
                (temp_cap <= max_market_cap)
            ]
