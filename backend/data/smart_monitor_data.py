...
class SmartMonitorDataFetcher:
    ...
    def __init__(self, use_tdx: bool = None, tdx_base_url: str = None):
        ...
        # TDX数据源配置
        if use_tdx is None:
            from config.config import TDX_CONFIG
        ...
        if tushare_token:
            try:
                # 显式设置 Tushare 缓存目录，避免权限问题
                os.environ['TS_DATA_DIR'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.tushare')
    ...
    def get_realtime_quote(self, stock_code: str, retry: int = 1) -> Optional[Dict]:
        ...
        # 将带后缀的股票代码转为纯数字（因为 TDX 和 AKShare 多以纯数字进行内部转换）
        ...
        if self.use_tdx and self.tdx_fetcher:
            try:
                ...
                if quote:
                    ...
                    self.logger.warning(f"TDX获取失败 {clean_code}，尝试降级到AKShare")
            except Exception as e:
                self.logger.warning(f"TDX获取异常 {clean_code}: {e}，尝试降级到AKShare")
        ...
        for attempt in range(retry):
            try:
                ...
                # 1.2 获取分钟级实时行情
                min_df = ak.stock_zh_a_hist_min_em(symbol=clean_code, period='1', adjust='')
                
                if min_df.empty:
                    self.logger.warning(f"AKShare未找到股票 {clean_code} 的分钟行情数据")
                    if attempt < retry - 1:
                        time.sleep(2)
                        continue
                    break
                ...
                # 1.3 获取历史数据（计算昨收）
                hist_df = ak.stock_zh_a_hist(symbol=clean_code, period='daily', adjust='')
                ...
                self.logger.info(f"✅ AKShare成功获取 {clean_code} ({stock_name}) 实时行情")
                ...
                if attempt < retry - 1:
                    self.logger.warning(f"AKShare获取失败 {clean_code}，第{attempt+1}次重试... 错误: {type(e).__name__}: {str(e)[:50]}")
                    time.sleep(2)  # 等待2秒后重试
                else:
                    self.logger.warning(f"AKShare获取失败 {clean_code}（已重试{retry}次），尝试降级")
        
        # 降级到Tushare
        if self.ts_pro:
            self.logger.info(f"降级到Tushare获取 {clean_code}...")
            return self._get_realtime_quote_from_tushare(clean_code)
        else:
    ...
    def _get_technical_indicators_uncached(
        ...
        if allow_tdx and self.use_tdx and self.tdx_fetcher:
            try:
                ...
                self.logger.warning(f"TDX计算技术指标异常 {clean_code}: {e}，尝试降级到AKShare")
        ...
        for attempt in range(retry):
            try:
                ...
                if df.empty or len(df) < 60:
                    if attempt < retry - 1:
                        self.logger.warning(f"AKShare历史数据不足 {clean_code}，第{attempt+1}次重试...")
                        time.sleep(1)
                        continue
                    else:
                        self.logger.warning(f"AKShare历史数据不足 {clean_code}，尝试降级")
                        break
                ...
                if attempt < retry - 1:
                    self.logger.warning(f"AKShare获取历史数据失败 {clean_code}，第{attempt+1}次重试... 错误: {type(e).__name__}: {str(e)[:50]}")
                    time.sleep(1)
                else:
                    self.logger.warning(f"AKShare获取历史数据失败 {clean_code}（已重试{retry}次），尝试降级到Tushare")
                    break
        
        # 方法3: 降级到Tushare
        if self.ts_pro:
            self.logger.info(f"降级到Tushare获取 {clean_code} 历史数据...")
            return self._get_technical_indicators_from_tushare(clean_code, period)
        else:
            self.logger.error(f"AKShare失败且未配置Tushare，无法获取 {clean_code} 技术指标")
            return None
